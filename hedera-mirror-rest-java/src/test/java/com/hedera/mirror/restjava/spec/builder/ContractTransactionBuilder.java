...
@Named
public class ContractTransactionBuilder
        extends AbstractEntityBuilder<ContractTransaction, ContractTransaction.ContractTransactionBuilder> {
    ...
    public ContractTransactionBuilder(
            ContractResultBuilder contractResultBuilder,
            ContractStateChangeBuilder contractStateChangeBuilder,
            ContractLogBuilder contractLogBuilder) {
    ...
    @Override
    protected ContractTransaction getFinalEntity(
            ContractTransaction.ContractTransactionBuilder builder, Map<String, Object> entityAttributes) {
    ...
    protected Supplier<List<Map<String, Object>>> getSpecEntitiesSupplier(SpecSetup specSetup) {
        ...
        if (contractResults != null) {
            for (var result : contractResults) {
                ...
                options.add(getContractTransaction(
                        contractResult.getContractId(),
                        contractResult.getPayerAccountId().getId(),
        ...
        if (contractStateChanges != null) {
            for (var result : contractStateChanges) {
                ...
                options.add(getContractTransaction(
                        stateChange.getContractId(),
                        stateChange.getPayerAccountId().getId(),
                        stateChange.getConsensusTimestamp()));
            }
        }
        ...
        return () -> options.stream()
                .collect(Collectors.groupingBy(
                        item -> item.getOrDefault("consensus_timestamp", DEFAULT_CONSENSUS_TIMESTAMP)))
                .entrySet()
                .stream()
    ...
    private Map<String, Object> getContractTransaction(long entityId, long payerAccountId, long consensusTimestamp) {
        return new HashMap<>(Map.of(
                "entity_id", entityId, "consensus_timestamp", consensusTimestamp, "payer_account_id", payerAccountId));
    }

    private Collection<Map<String, Object>> getTransactionsForEntry(
            Object timestamp, List<Map<String, Object>> candidates) {
