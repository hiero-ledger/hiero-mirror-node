...
final class NetworkControllerTest extends ControllerTest {
    ...
    //            final var expected = feeScheduleMapper.map(feeSchedule, exchangeRate, Bound.EMPTY,
    // Sort.Direction.ASC);
    //
    //            // when
    ...
    //            final var exchangeRateSet = exchangeRateSet(1); // Use consistent hbarEquiv for predictable gas
    // calculations
    //            return new SystemFile<>(exchangeRateFile(exchangeRateSet.toByteArray()), exchangeRateSet);
    //        }
    //    }
    ...
    //            assertThat(actual.getNodes()).isNotNull().hasSize(2);
    //            assertThat(actual.getNodes())
    //                    .extracting(NetworkNode::getNodeId)
    ...
    final class RegisteredNodesEndpointTest extends EndpointTest {
        ...
        void filteredByType() {
            ...
            // when
            final var actual =
                    restClient.get().uri("?type=BLOCK_NODE").retrieve().body(RegisteredNodesResponse.class);
        ...
        void orderedDesc() {
            ...
            // when
            final var actual = restClient.get().uri("?order=desc").retrieve().body(RegisteredNodesResponse.class);
        ...
        void nextLinkIsPresent() {
            ...
            // when
            final var actual = restClient.get().uri("?limit=1").retrieve().body(RegisteredNodesResponse.class);
