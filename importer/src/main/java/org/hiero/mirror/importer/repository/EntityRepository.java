...
public interface EntityRepository extends CrudRepository<Entity, Long> {
    ...
    @Query(value = "select evm_address,id from entity where id in (?1) and length(evm_address) > 0")
    List<EvmAddressMapping> findEvmAddressesByIds(Iterable<? extends Long> ids);

    @Modifying
    @Query(value = "update entity set type = 'CONTRACT' where id in (:ids) and type <> 'CONTRACT'")
