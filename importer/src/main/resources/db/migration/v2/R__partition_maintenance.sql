...
values
  ('account_balance', ${balancePartitionTimeInterval}, ${partitionStartDate}::timestamptz)
...
insert into partition_config (parent_table, partition_interval, start_from)
values
  ('token_balance', ${balancePartitionTimeInterval}, ${partitionStartDate}::timestamptz)
on conflict (parent_table) do update
set partition_interval = excluded.partition_interval,
    start_from         = excluded.start_from;

create or replace procedure create_mirror_node_time_partitions()
language plpgsql
as
$$
declare
...
begin
  ...
  loop
    if current_timestamp + partition_info.partition_interval < partition_info.next_from then
