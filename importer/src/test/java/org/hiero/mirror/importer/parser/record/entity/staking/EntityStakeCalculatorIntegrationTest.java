...
class EntityStakeCalculatorIntegrationTest extends ImporterIntegrationTest {
    ...
    @Test
    void calculateWithMultipleChunks() {
        // given - chunk size of 2 forces two intermediate chunks for account1/account2 and account3/account4,
        // plus a final chunk for the staking reward account. account3 and account4 proxy-stake to account2,
        // but are processed in the chunk after account2 — verifying that entity_state_proxy_staking is
        // pre-computed once per period and correctly applied across chunk boundaries.
        entityProperties.getPersist().setPendingRewardChunkSize(2);
        ...
        // account1 staked to node 1 for two periods — earns accumulated rewards
        var account1 = domainBuilder
                .entity()
                .customize(e -> {
                    long stakePeriodStart = epochDay - 2;
        ...
        // account2 staked to node 2; account3 and account4 proxy-stake to it
        var account2 = domainBuilder
                .entity()
                .customize(e -> {
                    long createdTimestamp =
        ...
        // account3 stakes to account2 (no node staking of its own)
        var account3 = domainBuilder
                .entity()
                .customize(e -> {
                    long createdTimestamp =
        ...
        // account4 declined rewards but stakes to account2, so still contributes to proxy staking
        var account4 = domainBuilder
                .entity()
                .customize(e -> {
                    long createdTimestamp =
        ...
        var expectedEntityStake1 = fromEntity(account1)
                .customize(es -> es.endStakePeriod(epochDay)
                        ...
                        .timestampRange(Range.atLeast(nodeStakeTimestamp)))
                .get();
        // stakedToMe must include account3 and account4 even though they land in a later chunk than account2
        var expectedEntityStake2 = fromEntity(account2)
                .customize(es -> es.endStakePeriod(epochDay)
                        .stakedToMe(account3BalanceStart + account4Balance)
                        .stakeTotalStart(account2BalanceStart + account3BalanceStart + account4Balance)
                ...
                .untilAsserted(() -> {
                    assertThat(entityStakeRepository.findAll())
                            .containsExactlyInAnyOrder(
                                    expectedEntityStake1, expectedEntityStake2, expectedEntityStake800);
                    // progress rows must be cleaned up after successful period completion
                    assertThat(jdbcOperations.queryForList("select * from entity_stake_calculation_state"))
                            .isEmpty();
                });
        entityStake1.setTimestampUpper(nodeStakeTimestamp);
        entityStake800.setTimestampUpper(nodeStakeTimestamp);
