...
public interface EntityRepository extends CrudRepository<Entity, Long> {
     ...
     * @return an Optional containing the entity's state at the specified timestamp. If there is no record found for the
    ...
    /**
     * Retrieves the most recent state of an entity by its ID up to a given block timestamp. The method considers both
     * the current state of the entity and its historical states and returns the one that was valid just before or equal
     * to the provided block timestamp. It performs a UNION operation between the 'entity' and 'entity_history' tables,
     * filters the combined result set to get the records with a timestamp range less than or equal to the provided
     * block timestamp and then returns the most recent record.
     *
    ...
    @Query(value = "select coalesce(max(e.id), ?1) from entity e where e.id >= ?1", nativeQuery = true)
    Long findMaxId(Long firstUserEntity);
}
