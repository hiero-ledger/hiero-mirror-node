...
import {AccessListItem, AuthorizationListItem, TransactionResult} from '../model';
...
class ContractResultDetailsViewModel extends ContractResultViewModel {
  ...
  constructor(
    ...
    if (!isNil(ethTransaction)) {
      this.access_list = ethTransaction.accessList?.map((item) => new AccessListItem(item)) ?? null;
